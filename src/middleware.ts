...
export const onRequest = defineMiddleware(async (contexto, proxima) => {
  ...
  if (!supabaseConfigurado) {
    if (caminho === '/admin/entrar') return proxima();
    if (ehApiAdmin) {
      return new Response(JSON.stringify({ erro: 'Blog ainda não configurado.' }), {
  ...
  if (!user) {
    if (telaDeLogin) { (locals as any).supabase = supabase; return proxima(); }
    if (ehApiAdmin) {
      ...
      return new Response(JSON.stringify({ erro: 'Sessão expirada.' }), {
        status: 401, headers: { 'Content-Type': 'application/json' },
      });
    }
    const destino = encodeURIComponent(caminho + url.search);
    return redirect(`/admin/entrar?proximo=${destino}`, 302);
  }
  ...
  if (!perfil || !(perfil as Perfil).ativo) {
    ...
    if (ehApiAdmin) {
      return new Response(JSON.stringify({ erro: 'Acesso desativado.' }), {
        status: 403, headers: { 'Content-Type': 'application/json' },
      });
    }
    return redirect('/admin/entrar?motivo=inativo', 302);
  }

  if (telaDeLogin && caminho === '/admin/entrar') return redirect('/admin/posts', 302);

  (locals as any).supabase = supabase;
  (locals as any).perfil = perfil as Perfil;
  (locals as any).usuarioId = user.id;

  const resposta = await proxima();
  // O admin nunca pode ser indexado nem ficar em cache compartilhado.
